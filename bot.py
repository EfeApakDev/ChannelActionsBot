...
**Ben Kanal İstek Onay Botuyum, esas olarak yeni ile çalışmaya odaklanan bir bot [yönetici onayı davet bağlantıları](https://t.me/telegram/153).**
...
start_buttons = [
    [Button.inline("beni nasıl kullanırsın❓", data="helper")],
...
async def settings(event):
    ...
    await event.edit(
        f"Ayarlar güncellendi! Kanala yeni üyeler `{chat}` Olacak {op}d!"
    )
...
async def approver(event):
    ...
    chat_welcome = (
        ...
        or "Merhaba {name},  katılma isteğiniz {chat} oldu  {dn}"
    )
    chat_welcome += "\nSend /start daha fazlasını öğrenmek için ."  # \n\n__**Powered by @BotzHub**__"
